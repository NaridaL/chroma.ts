...
convertsCorrectly = (m) ->
    topic: () ->
        try return chroma this.context.name
        catch e then return chroma JSON.parse this.context.name
    'color.hex() == chroma.${m}(color.${m}()).hex()': (color) -> assert.equal (chroma[m] color[m]()).hex(), color.hex()
...
vows
    ...
    .addBatch
        ...
        'hsv black':
            ...
            'hue is finite': (topic) -> assert isFinite topic[0]
        ...
        'hcl.h is 0 for hue-less colors':
            'black': convertsCorrectly('hcl')
            'grey': convertsCorrectly('hcl')
            'white': convertsCorrectly('hcl')
            'lightgreen': convertsCorrectly('hcl')
